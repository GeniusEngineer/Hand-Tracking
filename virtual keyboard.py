...
# Add this flag at the beginning, before the main loop
caps_lock = True
...
keys.append(Key(startX + 25, startY + 4 * h + 20, 5 * w, h, "CAPSLK"))
...
while True:
            ...
            if k.isOver(mouseX, mouseY) or k.isOver(signTipX, signTipY):
                ...
                if k.isOver(clickedX, clickedY):
                    if k.text == '<--':
                        ...
                        for k in keys:
                            if k.text.isalpha():
                                k.text = k.text.lower()  # Convert text to lowercase
                    elif k.text == 'capslk':
                        ##caps_lock = not caps_lock  # Toggle caps lock state
                        for k in keys:
                            if k.text.isalpha():
                                k.text = k.text.upper()  # Convert text to lowercase
                    ...
                    if clickTime - previousClick > 0.4:
                        if k.text == '<--':
                            ...
                            if k.text == 'SPACE':
                                ...
                                textBox.text += k.text if not caps_lock else k.text.upper()
                                # Simulating the press of the actual keyboard
                                keyboard.press(k.text if not caps_lock else k.text.upper())
    ...
    if pressedKey == ord('q'):
        break
    elif pressedKey == ord('c'):  # For example, 'c' key can toggle caps lock
        caps_lock = not caps_lock  # Toggle caps lock state
        for k in keys:
            if k.text.isalpha():
                if caps_lock:
                    k.text = k.text.upper()  # Convert text to uppercase
                else:
                    k.text = k.text.lower()  # Convert text to lowercase
